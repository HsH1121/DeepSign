...
def compose_hangul(label_check_compose_list):
    global final_inputed_labels, inputed_moum, last_confirmed_label  # ✅ 맨 위에 위치시켜야 함
    ...
    if (
        ...
        final_inputed_labels += hgtk.letter.compose(*label_check_compose_list)
        last_confirmed_label = label_check_compose_list[-1]
    else:
        final_inputed_labels += "".join(label_check_compose_list)
        last_confirmed_label = label_check_compose_list[-1]
...
while cap.isOpened():
    ...
    if (
        ...
        frame = draw_text_with_pil(frame, final_inputed_labels + "".join(map(str, label_check_compose_list)), (10, 80), color=(255, 255, 0))  # 완성 문자 출력
